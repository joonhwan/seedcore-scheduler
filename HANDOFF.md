...
## 1. 현재 상태 (M0 + M1 + M2a + M2b + M2c + M3a + M3b + M3c + M3d + v1.2 + v1.3 + v1.4 + v1.5 + v1.6 + 2026-09 요청 1·2·3차 완료)
...
> **배포 전에 남은 일 둘.**
> 1. **사용설명서(`docs/user-guide.md`)에 2차·3차 기능이 아직 없다.** 그룹 관리, 사용자 상세,
>    "퇴사자 포함" 보기, 계정 정리 섹션이 §8.4 의 열거에서 빠져 있다. 두 차수가 하나의 배포로 나가므로
>    **배포 전에 한 번에** 채운다.
> 2. **배포판 예행 연습(2차 계획의 Task 13)이 최신 코드로 수행되지 않았다.** `dist-release/` 의 zip 은
>    커밋 `1b8cde6`(2026-09-03) 기준이다. 이번 배포에는 마이그레이션이 포함되므로
>    `sp-server.exe` 가 종료 코드 3으로 멈추는지, `sp-migrate.exe` 가 `backups/pre-migrate/` 에 스냅샷을
>    뜨고 아홉 테이블의 행 수를 보존하는지 실제로 확인해야 한다.
>
> **다음 차수는 4차 — 프로젝트 목록 세로 스크롤과 간트 배율 10% 단위(요청 4·6)다.** 데이터베이스
> 변경이 없어 언제든 얹을 수 있다. ㉱(고객이 기존 내보내기를 써 본 뒤 줄 피드백)는 계속 보류이며,
> 6차 착수 전에 회신을 받는 편이 좋다.
