...
describe('historyLabelText', () => {
  it('진척율 내림', () => {
    expect(historyLabelText('UPDATE', { progress: { from: 80, to: 50 } })).toBe('진척율 80% → 50%');
  });
  it('진척율 올림', () => {
    expect(historyLabelText('UPDATE', { progress: { from: 50, to: 80 } })).toBe('진척율 50% → 80%');
  });
  it('진척율 100% 완료', () => {
    expect(historyLabelText('UPDATE', { progress: { from: 90, to: 100 } })).toBe('진척율 100%');
  });
  it('기간 변경(endAt)', () => {
