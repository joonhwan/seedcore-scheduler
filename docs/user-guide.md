...
간트 차트를 그림 파일로 저장해 보고서나 발표 자료(PPT)에 넣을 수 있습니다. 상단 툴바의 **내보내기 아이콘**(화면맞춤·오늘 버튼 오른쪽)을 누르고 **이미지로 내보내기 (PNG)**를 고르면 설정 창이 열립니다.
...
**내보내기**를 누르면 `프로젝트이름_간트_날짜.png` 파일이 내려받아집니다. 이 그림에는:

- 지금 화면 스크롤 위치와 상관없이 **프로젝트 전체 기간**이 한 장에 담기고,
- 고른 깊이까지 **일정이 모두 펼쳐진** 상태로 나오며,
- **오늘 날짜선(빨간 선)은 빠지고**,
- 화면의 두 배 해상도로 저장돼 확대해도 비교적 선명합니다.

> 내려받은 PNG는 PPT나 문서에 바로 붙여넣을 수 있습니다.
