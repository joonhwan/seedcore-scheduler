...
> 일정 트리는 최대 5단계 깊이까지만 만들 수 있습니다 (`depth` 0~4). 더 깊게 넣으려 하면 "최대 깊이를 초과하여..." 안내가 뜨고 자식 추가가 막힙니다.
...
행에 마우스를 올리면 나타나는 액션 버튼으로 조작합니다.

- **↑ 위로 / ↓ 아래로**: 같은 부모 안에서 순서를 바꿉니다.
- **⇄ 부모 변경**: 다른 그룹 아래로 옮깁니다. 열리는 대화상자에서 새 부모를 고르며, "◇ (루트로 이동)"을 고르면 최상위 노드로 뺄 수 있습니다. 자기 자신이나 자기 자손, 깊이 제한을 넘는 위치는 선택할 수 없으며 이유가 툴팁으로 안내됩니다.
...
- **현재 멤버 목록**: 이름·`@아이디`·역할 배지(MANAGER / MEMBER)가 보입니다. **제거** 버튼으로 멤버를 프로젝트에서 제외할 수 있습니다.
