...
판단하고 네 조합을 값으로 테스트할 수 있다). 두 exe 의 유일한 획득 경로는 같은 파일의
`acquireLock(role)` 이다: 확인 → 기록 → 되읽어 검증을 한 함수에 묶어 두어야 "확인 없이 기록" 하는
지름길이 생기지 않는다.
...
- **확인과 기록은 원자적이지 않다. 그래서 기록 후 되읽어 검증한다** (`acquireLock()`). 같은 순간에 뜬
  두 프로세스가 둘 다 'free' 를 읽고 둘 다 기록하면 파일에는 나중 쪽 PID 만 남는데, 소유권 기반 해제만
  으로는 먼저 쓴 쪽이 진행하는 것을 막지 못한다 — 나중 쪽이 끝나며 잠금을 지우는 순간 먼저 쓴 쪽은
  `DROP TABLE` 한복판인데 아무 표시도 없다. 기록 직후 파일에 자기 PID 가 남아 있는지 확인하면 이
  경우가 결정적으로 갈린다: 정확히 하나만 진행하고, 진 쪽은 사전 확인이 냈을 것과 같은 안내로 멈춘다.
  실제로 동시 기록이 없었다면 되읽기는 항상 자기 PID 이므로 정상 실행을 막지 않는다.
- 남는 창: 낡은 잠금 정리 경로(`readLock()` 으로 죽은 PID 확인 → 덮어쓰기)는 여전히 원자적이지 않다
  (`O_EXCL` 로 만들어도 낡은 잠금을 치우려면 결국 "확인 후 삭제" 가 필요해 같은 창이 남는다).
  다만 그 경쟁에서 지는 쪽도 위 되읽어 검증에 걸려 멈추므로 데이터 손실로 이어지지 않는다.
  "빈틈이 없다" 고 말할 수는 없고, 막으려는 것은 악의적 경쟁이 아니라 운영 실수다.
- 잠금 파일을 아예 만들지 못하면(권한/디스크) **막지 않고 진행한다.** 안전장치를 못 걸었다고 폐쇄망에서
  서버 시작이나 업그레이드 자체를 막으면 안전장치가 서비스 중단 사유로 바뀐다. 대신 경고 한 줄을
  화면과 `logs/` 양쪽에 남긴다 — 안전장치가 조용히 꺼진 채로 도는 상황을 사고 후에라도 알 수 있어야 한다.
...
- **PID 재사용은 실재하는 거짓 양성이다.** 윈도우가 그 PID 를 다른 프로그램에 재배정하면 생존 판정이
  ...
  핸들러를 지우거나, 소유권 확인 없이 해제하거나, 기록 후 되읽어 검증을 빼거나, 경로 안내를 빼는
  다섯 가지 변경은 각각 현장에서 업그레이드나 서버 시작을 막는다(또는 막아야 할 것을 통과시킨다).
...
| 단위 | `acquireLock()` — 경쟁이 없으면 확보(파일에 자기 PID), 기록 직후 **살아 있는 남의 PID** 가 파일을 차지하면 exit 7 (실제 자식 프로세스를 띄워 진짜 살아 있는 외부 PID 를 쓴다), 사전 확인에서 막히면 기록을 시도조차 하지 않음, 기록 실패(잠금 경로에 같은 이름의 디렉터리를 두어 EISDIR 유발) 시 경고와 함께 `held: false` 로 진행 |
...
| 잠금 획득 방식 | 확인 → 기록 → 되읽어 검증 (`acquireLock()` 한 경로) | 확인과 기록이 원자적이지 않아, 같은 순간 뜬 두 프로세스가 둘 다 통과할 수 있었다. 되읽어 검증은 세 줄이고 결정적이며, 실제 동시 기록이 없으면 발동하지 않으므로 정상 실행을 막을 위험이 없다. 낡은 잠금 정리 경로의 잔여 창은 남지만 그 경쟁의 패자도 이 검증에 걸려 데이터 손실로 이어지지 않는다 (§6) |
| 잠금 기록 실패 시 | 막지 않고 진행 + 경고 한 줄 | 안전장치를 못 걸었다고 폐쇄망에서 업그레이드나 서버 시작을 막으면 안전장치가 서비스 중단 사유가 된다. 대신 화면과 `logs/` 에 경고를 남겨 조용한 성능 저하가 되지 않게 한다 |
