...
그래서 `README-exe.txt` 의 "1) sp-server.exe 를 종료합니다" 한 줄에 의존하지 않고, **PID 잠금 파일**로
기계적으로 막는다 (`apps/api/src/common/server-lock.ts`).

- `sp-server.exe` 는 부팅 시 DB 파일 옆(`<DB 폴더>/sp-server.lock`)에 자기 PID 를 쓰고, 종료 시 지운다.
  잠금 경로는 `resolveDbFilePath()` 에서 파생시킨다 — 두 exe 가 같은 파일을 보는 것이 이 방어선의
  전제이므로, `resolveDatabaseUrl()` 을 공용화한 것과 같은 이유로 경로 계산도 한 곳에 둔다.
- `sp-server.exe` 는 자기 잠금을 **검사하지 않는다.** 서버 두 개가 같은 포트를 다투는 문제는 별개다.
- `sp-migrate.exe` 는 **백업보다 먼저** 잠금을 확인하고, 살아 있으면 exit 7 로 물러난다. 순서가 중요하다 —
  안내가 "DB 를 전혀 변경하지 않았습니다" 라고 약속하기 때문이다. 판정(`decideMigrate()`)보다는 뒤에
  둔다: 적용할 것이 없는 상태에서 서버가 켜져 있는 것은 정상이고, 그때는 "이미 최신입니다" 로 끝나야 한다.
...
- **PID 재사용은 실재하는 거짓 양성이다.** 윈도우가 그 PID 를 다른 프로그램에 재배정하면 생존 판정이
  참이 되어 관리자가 갇힌다. 그래서 안내 문구가 잠금 파일의 전체 경로를 싣고 "서버를 종료한 것이
  확실하면 이 파일을 지우고 다시 실행하라" 고 알려준다. **이 탈출구가 없으면 이 안전장치가 정상적인
  업그레이드를 브릭(brick)시킨다** — 폐쇄망에는 원격으로 손봐줄 사람이 없다. 잠금을 "단순화" 하려는
  다음 사람은 이 문단을 먼저 읽어야 한다: 존재 검사만으로 바꾸거나, 시그널 핸들러를 지우거나,
  경로 안내를 빼는 세 가지 변경은 각각 현장에서 업그레이드를 막는다.
...
3. 서버 실행 중  → 잠금 파일의 PID 가 살아 있으면 안내 + exit 7 (DB 를 열기만 하고 물러난다)
4. 백업          → backups/pre-migrate/sam_YYYYMMDD_HHMMSS.db  (VACUUM INTO)
5. 순차 적용     → 마이그레이션 이름을 하나씩 출력
6. 성공          → 요약 + "이제 sp-server.exe 를 실행하십시오", exit 0
7. 실패          → 실패 지점 + 백업 파일 경로로 복원 안내, exit 1
```
...
| 7 | `sp-server.exe` 실행 중 (잠금 파일의 PID 가 살아 있음) | `formatServerRunningNotice` | **전혀 변경되지 않음.** 백업조차 만들지 않는다 (§6 참고). 안내에 잠금 파일 경로와 "지우고 재실행" 탈출구를 함께 싣는다 |
...
| 단위 | `server-lock.ts` — 잠금 경로 파생(상대경로/쿼리스트링 정규화), 쓰기·읽기·삭제 왕복, 깨진 내용은 "잠금 없음", 죽은 PID 는 막지 않음 |
| 통합 | `runMigrate()` 서버 실행 중 경로 — 살아 있는 PID(테스트 프로세스 자신)로 잠금을 만들면 exit 7 이고 **`backups/` 폴더가 생기지도 않는다**. 검사가 백업보다 먼저라는 순서를 이 단언으로 고정한다. 낡은 잠금(죽은 PID)은 막지 않는다는 짝 테스트도 둔다 |
...
| 서버 실행 중 적용 방지 | PID 잠금 파일 + 생존 확인, exit 7 | 트랜잭션을 쓰지 않는 대가로, 적용 중 서버가 커밋한 편집은 사전 백업에도 없어 복구가 불가능하다(§6). 문서 한 줄("서버를 종료하십시오")로 막을 위험이 아니다. 존재가 아니라 생존으로 판단하고, PID 재사용 거짓 양성을 위해 "잠금 파일을 지우고 재실행" 탈출구를 안내에 싣는다 |
