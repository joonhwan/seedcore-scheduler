...
**앞뒤 공백 처리.** `canSubmitProjectName` 이 trim 후 비교하면 `"기획"` → `" 기획 "` 은 변경으로
치지 않아 버튼이 비활성된다. 서버는 trim 을 하지 않으므로 공백 포함 문자열을 그대로 보내면 DB 에
`" 기획 "` 이 저장된다. 이름 앞뒤 공백을 허용할지는 데이터 품질 판단이므로 사용자가 직접 결정하고
`canSubmitProjectName` 본문을 작성한다.
